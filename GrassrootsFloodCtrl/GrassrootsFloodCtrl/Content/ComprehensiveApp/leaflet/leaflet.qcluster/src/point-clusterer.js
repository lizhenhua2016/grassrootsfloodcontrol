...
var QCluster = (function(module){
	...
	function processCustomData(data) {
	    ...
	    for (i = len - 1; i >= 0; --i) {
	        ...
	        lng = pointObj.poslon;
	        lat = pointObj.poslat;
	        var tempPoint = new L.Point(lat, lng);
	        tempPoint.options = pointObj;
	        pointData.push(tempPoint);
	    }
	...
	module.PointClusterer = function(pointArr, layerId, map, clusterCssClass, opts){
		...
		} else if (this.dataFormat === 'custom') {
		    this.pointData = processCustomData(pointArr);
        } else { // point array
	...
	module.PointClusterer.prototype.makeClusters = function(map, layer, clusterTolerance, mapBounds) {
        ...
        // Cluster the points
		clusterArr = module.clusterPoints(this.pointData, mapBounds, resolution, this.tolerance);
			...
			//if(module.Utils.withinBounds(clusterArr[i].cX, clusterArr[i].cY, webMercMapBounds, resolution)) {
				...
				if (cnt === 1) {
					...
					if(this.reportingProperty !== null && points[0][this.reportingProperty]) {
						...
						classificationIds = points[0].options[this.reportingProperty].toString().split(',');
				...
				clusterMarker = L.marker([clusterArr[i].cX, clusterArr[i].cY], { icon: myIcon });
				if(commonModule.projectName(this.projectType))
				    clusterMarker.bindLabel(commonModule.basinName(clusterArr[i].id + 1) + '-' + commonModule.projectName(this.projectType), { direction: 'left', noHide: true });
				
                clusterMarker["points"] = points;
                clusterMarker["pointIds"] = (i + 1);
				...
				if (i < 7) clusterMarkers.push(clusterMarker);
		...
		if (commonModule._projectCollection[this.projectType]) {
		    if ($.isEmptyObject(commonModule._projectCollection[this.projectType].superLayer))
		        commonModule._projectCollection[this.projectType].superLayer = this.layer;
		}
		...
		if(this.layerVisibility){
            ...
            //if (this.map.hasLayer(commonModule._projectCollection[this.projectType].superLayer)) {
            //    commonModule._projectCollection[this.projectType].superLayer.bringToFront();
		    //}

            if (this.map.hasLayer(application.emapLayer)) {
                application.emapLayer;
            } else if (this.map.hasLayer(application.domLayer)) {
                var tempLayerArray = [];
                for (var l in application.domLayer._layers)
                    tempLayerArray.push(l);
                for (var i = 1; i >= 0; i--)
                    application.domLayer._layers[tempLayerArray[i]].bringToBack()
            } else {
                application.topoLayer;
            }
	...
	module.PointClusterer.prototype.mapMove = function(){
		...
		var tempCollection = commonModule._projectCollection[this.projectType];
		if (tempCollection) {
		    if (this.map.getZoom() < 9) {
		        // 当缩放到一定级别的时候去除次一级的图标

		        if (!$.isEmptyObject(tempCollection.superLayer) && !this.map.hasLayer(tempCollection.superLayer))
		            this.map.addLayer(tempCollection.superLayer);
		        if (this.map.hasLayer(tempCollection.currentSubLayer))
		            this.map.removeLayer(tempCollection.currentSubLayer);
		        if (this.map.hasLayer(tempCollection.lineLayer))
		            this.map.removeLayer(tempCollection.lineLayer);
		        if (this.reportingProperty && this.reportingProperty != "") {
		            this.makeDonuts();
		        }
		    } else {
		        // 从地图上去除聚类图标
		        if (this.map.hasLayer(tempCollection.superLayer))
		            this.map.removeLayer(tempCollection.superLayer);
		        if (!$.isEmptyObject(tempCollection.currentSubLayer) && !this.map.hasLayer(tempCollection.currentSubLayer)) {
		            this.map.addLayer(tempCollection.currentSubLayer);
		        }
		    }
		}
		//if (this.map.hasLayer(commonModule._projectCollection[this.projectType].superLayer)) {
    ...
    module.PointClusterer.prototype.defaultMouseoverHandler = function(e) {
    	...
    	for(var i=len-1; i>=0; --i) {
    		...
    		var latLng = L.latLng([p.x || p.lat, p. y || p.lng]);
