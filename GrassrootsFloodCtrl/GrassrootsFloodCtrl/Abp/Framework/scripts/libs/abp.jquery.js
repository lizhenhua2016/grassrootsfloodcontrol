        ...
        return $.Deferred(function ($dfd) {
            $.ajax(options)
                .done(function (data) {
                    ...
                    abpAjaxHelper.handleData(data.responseJSON, userOptions, $dfd);
        ...
        handleData: function (data, userOptions, $dfd) {
            if (data) {
                if (typeof (data.responseStatus) === 'undefined') {
                    $dfd && $dfd.resolve(data, data);
                    userOptions.success && userOptions.success(data, data);
                } else { //data.success === false
                    var messagePromise = null;
                    if (data.responseStatus.message) {
                        messagePromise = abp.message.error(data.responseStatus.message);

                        $dfd && $dfd.reject(data.responseStatus.message);
                        userOptions.error && userOptions.error(data.responseStatus.message);
                    }
