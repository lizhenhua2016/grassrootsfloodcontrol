        ...
        function createEdit(operateCol) {
            $(editLink).appendTo(operateCol).on("click", function () {
                if (replaceQuote($(this).html()) == replaceQuote(c.edit)) { //如果此时是编辑状态 
                    ...
                    for (var i = 0; i < cols.length; i++) {
                        ...
                        var tdtype = tr.attr("data-type");
                        var inputValue = "";
                        switch(tdtype){
                            case "select":
                                inputValue = tr.children('select').val();
                                break;
                            case "textarea":
                                inputValue = tr.children('textarea').val();
                                break;
                            default:
                    ...
                    data.push(p.find("th").attr("data-id"));
            ...
            function toSave(ele) {
                ...
                for (var i = 0; i < cols.length; i++) {
                    ...
                    switch (tdtype) {
                        case "select":
                            editTr = "<select class=\"form-control\"><option>" + tr.html() + "</option></select>";
                            break;
                        case "textarea":
                            editTr = "<textarea class=\"form-control\">" + tr.html() + "</textarea>";
                            break;
                        default:
            ...
            function toEdit(ele, isSave) {
                ...
                for (var i = 0; i < cols.length; i++) {
                    ...
                    if (isSave) {
                        switch (tdtype) {
                            case "select":
                                value = tr.children('select').val();
                                break;
                            case "textarea":
                                value = tr.children('textarea').val();
                                break;
                            default:
        ...
        function createAdd(operateCol) {
            $(addLink).appendTo(operateCol).on("click", function () {
                ...
                for (var i = 0; i < childLen; i++) {
                    if (i == 0) {
                        copyRow.children().eq(i).html("");
                    } else {
                        switch (copyRow.children().eq(i).attr("data-type")) {
                            case "select":
                                copyRow.children().eq(i).html("<select class=\"form-control\"><option></option></select>");
                                break;
                            case "textarea":
                                copyRow.children().eq(i).html("<textarea class=\"form-control\"></textarea>");
                                break;
                            default:
                ...
                var confirm = $(confirmLink).appendTo(last).on("click", function () {
                    ...
                    for (var i = 0; i < childLen; i++) {
                        if (i != c.operatePos) {
                            var v = "";
                            var tdtype = copyRow.children().eq(i).attr("data-type");
                            switch (tdtype) {
                                case "select":
                                    v = copyRow.children().eq(i).children("select").val();
                                    break;
                                case "textarea":
                                    v = copyRow.children().eq(i).children("textarea").val();
                                    break;
                                default:
                                    v = copyRow.children().eq(i).children("input").val();
                                    break;
                            }
                            data.push(v);
