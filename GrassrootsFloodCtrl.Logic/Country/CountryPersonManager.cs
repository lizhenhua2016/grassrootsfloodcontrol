    ...
    public class CountryPersonManager : ManagerBase, ICountryPersonManager
    {
        ...
        private List<CountryPerson> GetCountryPerson(List<InsertExeclCountryPerson> insertExeclCountryPersonList, CountryImportExcel request, int checkCount)
        {
            ...
            else
            {
                foreach (var insertExecl in insertExeclCountryPersonList)
                {
                    ...
                    {
                        countryPerson.UserName = insertExecl.UserName.Trim();
                        ...
                        countryPerson.Phone = insertExecl.Phone.Trim();
                        countryPerson.UpdateName = insertExecl.UserName.Trim();
            ...
            using (var db = DbFactory.Open())
            {
                ...
                newlist.ForEach(w =>
                {
                    ...
                    CountryPersonServiceModel cpsm = new CountryPersonServiceModel()
                    {
                        ...
                        CreateTime = f.FirstOrDefault().CreateTime,
                        Gwid = gwid,
                        
                        Remark = f.FirstOrDefault().Remark,
                ...
                return new BsTableDataSource<CountryPersonServiceModel> { rows = rlist, total = count };
            }
        }
        #endregion
            ...
            using (var db = DbFactory.Open())
            {
                ...
                if (requset.typeId == 6)
                {
                    ...
                    if (list.Count > 0)
                    {
                        ...
                        if (dt.Columns.Contains("Gwid")) dt.Columns.Remove("Gwid");
        ...
        public bool SaveCountryPerson(SaveCountryPerson requset)
        {
            ...
            using (var db = DbFactory.Open())
            {
                ...
                //}
                info.UserName = requset.name.Trim();
                info.CreateTime = DateTime.Now;
                info.Phone = requset.Mobile.Trim();
                ...
                if (requset.Id != 0)
                {
                    ...
                    else
                    {
                        ...
                        if (result && (oldInfo.Phone.Trim() != info.Phone || oldInfo.UserName.Trim() != info.UserName || oldInfo.adcd != info.adcd))
                        {
                            _IAppRegPersonUpdate.AppRegPersonSaveOne(new AppRegPersonSaveOne() { adcdid = adcdInfo.Id, username=info.UserName, hanphone = info.Phone });
                        }
                        return result;
                    }
                }
        ...
        public bool DelectCountryPerson(DeleteCountryPerson request)
        {
            ...
            using (var db = DbFactory.Open())
            {
                ...
                if (result)
                {
                    _IAppRegPersonUpdate.AppRegPersonDelMore(new AppRegPersonDelMore() { countyadcd=_AdcdItems.adcd, AdcdIds = _ListAdcdItems });
                }
                return result;
            }
        }
